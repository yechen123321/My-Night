...
import { TextError } from '/src/api/Student'
...
const submitQuestion = async () => {
  axios.post('http://47.121.186.98:8084/question/text', {
    text: text.value
  }, {
    headers: {
...
const Ifly = async () => {
  axios.post('http://47.121.186.98:8084/question', {
    id: '2',
    name: '朱耿键',
    question: text.value + '给出其中文本纠错, 按照一段原文+建议修改的格式给我'
  }, {
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  <div class='ExamMain-translate'>
    ...
    <div class='ExamBody'>
      ...
      <div class='WorkBody'>
        ...
        <div class='TextBox'>
          <textarea v-model='text' class='Text' name='' id='' cols='30' rows='10'></textarea>
        </div>
        <div class='down'>
          <div class='next' @click='Ifly'>文本纠错</div>
        </div>
      </div>
    </div>
  </div>
    ...
    .WorkBody {
      ...
      .TextBox {
        ...
        .S {
          ...
          font-size: 0.9vw;
          font-weight: bolder;
          background: rgba(223, 225, 225, 0.7);
        }
